...
from .macos_types import Platform, Target
...
def build(options):
    ...
    for python_target in targets_to_build:
        is_abi3 = python_target.python_binder == PythonBinder.NANOBIND
        ...
        environment["DRAKE_IS_ABI3_WHEEL"] = str(int(is_abi3))
...
def add_selection_arguments(parser):
    ...
    parser.add_argument(
        ...
        default=",".join(
            sorted(
                set([t.platform(Role.BUILD).python_tag for t in PYTHON_TARGETS])
            )
        ),
        help=(
